          ...
          <div style={{display:'flex',flexDirection:'column',gap:20}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              {(activeTab==='overview'||activeTab==='dialogue')&&(<>
                ...
                <IssueSection title="Extra dialogue (not in script)" icon="🚨" items={results.extraDialogue} renderItem={m=>(<><Badge level={m.severity||'warning'}/><div><div style={{color:'#f472b6',fontSize:12,fontStyle:'italic',lineHeight:1.5}}>"{m.line||m.text||m.dialogue||m.content||(typeof m==='string'?m:JSON.stringify(m))}"</div>{(m.context||m.location||m.note)&&<div style={{fontSize:10,color:'#475569',marginTop:2}}>{m.context||m.location||m.note}</div>}</div></>)}/>
